...
if submitted and patient_name:
    ...
    if pred == 1:
        ...
        try:
            ...
            st.warning("📡 Unable to detect location. Please enter it manually below.")
            city_input = st.text_input("📍 Enter your city or PIN code for hospital suggestions")
            if city_input:
                safe_city = city_input.replace(" ", "+")
                manual_url = f"https://www.google.com/maps/search/cancer+hospitals+near+{safe_city}"
                st.markdown(f"🔎 [Find cancer hospitals near **{city_input}**]({manual_url})", unsafe_allow_html=True)
